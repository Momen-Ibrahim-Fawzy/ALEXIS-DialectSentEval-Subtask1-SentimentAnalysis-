"""
Subtask 1 -- check whether fewer training epochs generalize better on top of v19's
validated recipe (gold char-noise + mild-filtered round-3 pseudo-labels, real official
F1=0.8667, current best). Motivation: v19's training loss curve drops to near-zero by
epoch 6-7 (e.g. 0.044 -> 0.006 -> 0.003 -> 0.002 -> 0.002 -> 0.002 across epochs 5-10) and
essentially plateaus after that -- epochs 8-10 add minimal further optimization on the
training objective while plausibly just fitting training-set-specific noise harder, which
could hurt generalization to unseen dialects (the whole point of the char-noise mechanism).
Tests epochs=7 (clear post-convergence point, before the flattest part of the curve)
against the current fixed epochs=10.

Usage:
  CUDA_VISIBLE_DEVICES=0 conda run -n mo python3 char_noise_epoch_check.py
"""
import random
...
def train_backbone(name, model_name, combined_df, epochs):
    print(f"\n=== Training {name} ({model_name}) | n={len(combined_df)} | epochs={epochs} ===")
    re.seed_everything()
    ...
    model = re.train_loop(model, train_loader, epochs=epochs, class_weights=weights, technique="fgm", extra=extra)
...
def main():
    ...
    models = [train_backbone(name, model_name, combined_df, EPOCHS_TO_TEST) for name, model_name in THREE_WAY.items()]
    probs = eval_ensemble(models, holdout_df)
    f1 = f1_score(holdout_labels, probs.argmax(axis=1), average="macro")
    print(f"\nHeld-out macro-F1 (3-way, char-noise, epochs={EPOCHS_TO_TEST}): {f1:.4f}")
    for model, _ in models:
        del model
    torch.cuda.empty_cache()

    print(f"\nFor reference: v19 (epochs=10, same char-noise recipe) = 0.9583")
    margin = f1 - 0.9583
    print(f"Margin (epochs={EPOCHS_TO_TEST} - epochs=10): {margin:+.4f}")
    if margin >= 0.01:
        print(f"epochs={EPOCHS_TO_TEST} meaningfully beats epochs=10 -- worth building a submission with fewer epochs.")
    else:
        print(f"epochs={EPOCHS_TO_TEST} did NOT meaningfully beat epochs=10 -- stick with v19's 10-epoch recipe.")
